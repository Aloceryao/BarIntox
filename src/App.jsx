...
const calculateRecipeStats = (recipe, ingredients) => {
  ...
  const finalVol = totalVol; // 簡化計算，暫不計融水
  const finalAbv = finalVol > 0 ? (totalAlcoholVol / finalVol) * 100 : 0;
  const suggestedPrice =
    totalCost > 0 ? Math.ceil(totalCost / TARGET_COST_RATE / 10) * 10 : 0;
...
const Badge = ({ children, color = 'slate', className = '' }) => {
  ...
  return (
    <span
      className={`px-2 py-1 rounded text-[10px] tracking-wider font-medium select-none ${
...
const ChipSelector = ({
  ...
  const isSelected = (opt) =>
    single ? selected === opt : selected?.includes(opt);
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        <label className="text-xs text-slate-500 font-bold uppercase select-none">
          {title}
        </label>
      </div>
...
const SingleItemScreen = ({ ingredients, searchTerm }) => {
  const singles = ingredients.filter(
    (i) =>
      i.type === 'alcohol' &&
      (i.nameZh.includes(searchTerm) ||
    ...
    <div className="grid gap-3 w-full">
      {singles.map((ing) => {
        ...
        return (
          <div
            key={ing.id}
            className="bg-slate-800 p-4 rounded-xl border border-slate-700/50 w-full"
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              {sizes.map((size) => {
                ...
                return (
                  <div
                    key={size.label}
                    className="bg-slate-900/50 p-2 rounded border border-slate-700/50 flex justify-between items-center"
                  >
        ...
        <div className="flex px-4 border-b border-slate-800/50 w-full">
          {[
            ...
            <button
              ...
              className={`flex-1 pb-3 text-sm font-medium border-b-2 transition-colors select-none ${
        ...
        {recipeCategoryFilter === 'single' ? (
          ...
          filtered.map((recipe) => {
            ...
            return (
              <div
                key={recipe.id}
                onClick={() => setViewingItem(recipe)}
                className="group bg-slate-800 rounded-2xl overflow-hidden shadow-lg border border-slate-800 hover:border-slate-700 transition-all active:scale-[0.98] flex flex-row h-36 w-full"
              >
                ...
                <div className="flex-1 p-3 flex flex-col justify-between overflow-hidden">
                  <div>
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {recipe.baseSpirit && (
                        <Badge color="blue" className="scale-90 origin-left">
                          {recipe.baseSpirit.split(' ')[0]}
                        </Badge>
                      )}
...
const InventoryScreen = ({
  ...
  const filteredIngredients = ingredients.filter((i) =>
    categoryFilter === 'all' ? true : i.type === categoryFilter
  );
    ...
    <div className="pb-24 animate-fade-in w-full">
      <div className="sticky top-0 bg-slate-950/95 backdrop-blur z-20 border-b border-slate-800 shadow-md px-4 pt-safe pb-0">
        <div className="flex justify-between items-center mb-4 mt-4">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-slate-800 text-slate-200 px-4 py-2 rounded-full border border-slate-700 text-sm hover:bg-slate-700"
          >
            <Plus size={16} /> 新增
          </button>
        </div>
      ...
      <div className="p-4 space-y-2 w-full">
        {filteredIngredients.map((ing) => (
          <div
            key={ing.id}
            className="flex items-center justify-between p-3 bg-slate-800 rounded-lg border border-slate-800 hover:border-slate-600 transition-colors group w-full"
          >
            <div
...
const QuickCalcScreen = ({ ingredients }) => {
  ...
  const draftStats = calculateRecipeStats(
        ...
        {mode === 'single' ? (
          <>
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-lg text-center focus:border-amber-500 outline-none"
                    placeholder="0"
                  />
                </div>
                <div className="flex-1">
            ...
            <div className="flex gap-4 items-center bg-slate-800 p-4 rounded-xl border border-slate-700">
              <div className="text-xs text-slate-400 font-bold uppercase w-20">
                目標成本率設定:
              </div>
              <div className="flex items-center gap-2 flex-1">
                <div className="relative flex-1">
                  ...
                  />
                  <span className="absolute right-2 top-2 text-slate-500 text-xs">
                    %
                  </span>
                </div>
                <div className="relative flex-1">
              ...
              <div className="space-y-3">
                ...
                {[
                  ...
                  return (
                    <div
                      key={row.label}
                      className="grid grid-cols-4 gap-2 items-center bg-slate-800/50 p-3 rounded-lg border border-slate-800"
                    >
                      ...
                      <div className="text-right text-blue-400 font-bold font-mono">
                        ${priceB}
                      </div>
                    </div>
                  );
                ...
                {ingredients
                  ...
                  .map((ing) => (
                    <button
                      key={ing.id}
                      onClick={() =>
                        setDraftIngs([...draftIngs, { id: ing.id, amount: 30 }])
                      }
  ...
  return (
    <div className="fixed inset-0 z-40 bg-slate-950 flex flex-col animate-slide-up w-full h-full">
      <div className="bg-slate-900 px-4 py-3 border-b border-slate-800 flex justify-between items-center safe-top pt-safe-offset">
        <button
          onClick={onClose}
          className="text-slate-400 hover:text-white p-2 select-none"
        >
          取消
        </button>
        <h3 className="text-slate-100 font-bold">
          {item.id && item.nameZh ? '編輯項目' : '新增項目'}
        </h3>
        <button
          onClick={onSave}
          className="text-amber-500 font-bold hover:text-amber-400 p-2 select-none"
        >
          儲存
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-xs text-slate-500 font-bold uppercase ml-1">
              中文名稱
            </label>
            <input
              value={item.nameZh}
              onChange={(e) => setItem({ ...item, nameZh: e.target.value })}
              className="w-full bg-transparent border-b border-slate-700 py-2 text-xl text-slate-100 placeholder-slate-600 focus:border-amber-500 focus:outline-none"
              placeholder="例如: 內格羅尼"
            />
          </div>
          <div>
            ...
            <input
              ...
              className="w-full bg-transparent border-b border-slate-700 py-2 text-lg text-slate-400 placeholder-slate-700 focus:border-slate-500 focus:outline-none font-serif italic"
        ...
        {!isRecipe ? (
          <div className="bg-slate-900 p-4 rounded-xl space-y-4 border border-slate-800">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <input
                  type="number"
                  value={item.abv}
                  onChange={(e) => setItem({ ...item, abv: e.target.value })}
                  className="w-full bg-slate-800 rounded p-2 text-white border border-slate-700"
                />
              </div>
            </div>
            <div className="mt-2">
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="flex-1 bg-slate-800 rounded p-2 text-white border border-slate-700"
                >
                  {ingCategories.map((cat) => (
                ...
                {showAddCat ? (
                  <div className="flex items-center bg-slate-800 border border-slate-600 rounded">
                    <input
                      className="w-20 bg-transparent p-1 text-xs outline-none"
                      placeholder="新分類"
                      value={newCatName}
                      onChange={(e) => setNewCatName(e.target.value)}
                    />
                  ...
                  <button
                    ...
                    className="p-2 bg-slate-800 border border-slate-700 rounded text-slate-400 hover:text-white"
                  >
                    <Plus size={20} />
                  </button>
                )}
            ...
            {item.id && (
              <button
                onClick={() => requestDelete(item.id, 'ingredient')}
                className="w-full py-3 text-rose-500 text-sm border border-rose-900/50 rounded mt-4"
              >
                刪除此材料
              </button>
            )}
          ...
          <>
            <div className="space-y-4 bg-slate-900 p-4 rounded-xl border border-slate-800">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-slate-300 h-24 focus:border-amber-500 outline-none mt-2"
                  placeholder="例如: 前味帶有柑橘香氣，尾韻回甘..."
                />
              </div>
              <ChipSelector
                title="調製手法"
                options={availableTechniques}
                selected={item.technique}
                single={true}
                onSelect={(val) => setItem({ ...item, technique: val })}
                onAdd={(val) =>
              ...
              <ChipSelector
                ...
                selected={item.glass}
                single={true}
                onSelect={(val) => setItem({ ...item, glass: val })}
                onAdd={(val) => setAvailableGlasses([...availableGlasses, val])}
              />
              <ChipSelector
                title="風味標籤"
                options={availableTags}
                selected={item.tags || []}
                single={false}
                onSelect={(val) => setItem({ ...item, tags: val })}
                onAdd={(val) => setAvailableTags([...availableTags, val])}
              />
            </div>

            <div>
              <div className="flex justify-between items-end mb-2 border-b border-slate-800 pb-2">
                <label className="text-sm text-slate-400 font-bold uppercase">
                  配方成分
                </label>
                <div className="text-right">
                  ...
                  <div className="text-emerald-400 font-mono">
                    $
                    {Math.round(
                      calculateRecipeStats(item, ingredients).totalCost
                    )}
                  </div>
                </div>
              </div>
              <div className="space-y-3 mb-3">
                {item.ingredients.map((ingItem, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="flex items-center gap-3 bg-slate-800/50 p-2 rounded border border-slate-800"
                    >
                      ...
                      <input
                        type="number"
                        className="w-16 bg-slate-900 border border-slate-700 rounded text-center text-amber-400 p-1"
                      ...
                      <button
                        onClick={() => {
                          const newIngs = item.ingredients.filter(
                            (_, i) => i !== idx
                          );
                          setItem({ ...item, ingredients: newIngs });
                        }}
              ...
              <div className="bg-slate-800 rounded-lg border border-slate-700 p-2 mt-2">
                <div className="text-xs text-slate-500 mb-2 font-bold uppercase">
                  加入材料
                </div>
                <div className="relative mb-2">
                  <Search className="absolute left-2 top-2 text-slate-500 w-4 h-4" />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="number"
                  placeholder={`建議 $${
                    calculateRecipeStats(item, ingredients).suggestedPrice
                  }`}
                  value={item.customPrice}
                  onChange={(e) =>
                    setItem({ ...item, customPrice: e.target.value })
                  }
            ...
            <div>
              ...
              <textarea
                value={item.method}
                onChange={(e) => setItem({ ...item, method: e.target.value })}
                className="w-full bg-slate-800 border border-slate-700 rounded p-3 text-slate-300 h-32 placeholder-slate-600"
                placeholder="1. 冰鎮酒杯..."
              />
            </div>
          </>
  ...
  return (
    <div className="fixed inset-0 z-30 bg-slate-950 flex flex-col animate-scale-in overflow-y-auto w-full h-full">
      <div className="relative h-64 sm:h-80 w-full shrink-0">
        ...
        <button
          onClick={onClose}
          className="fixed top-4 left-4 z-50 bg-black/30 backdrop-blur p-2 rounded-full text-white hover:bg-black/50 transition mt-safe"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
          <h1 className="text-4xl font-serif text-white font-bold mb-1 shadow-black drop-shadow-lg">
            {item.nameZh}
          </h1>
          <div className="flex items-center gap-2">
            <p className="text-slate-300 text-lg italic font-serif opacity-90">
              {item.nameEn}
            </p>
            {item.baseSpirit && (
              <Badge color="blue" className="shadow-lg">
                {item.baseSpirit.split(' ')[0]}
              </Badge>
            )}
            {item.technique && (
              <Badge color="purple" className="shadow-lg">
                {item.technique}
              </Badge>
            )}
          </div>
      ...
      <div className="flex-1 p-6 space-y-8">
          ...
          <div className="text-center">
            ...
            <div
              className={`text-xl font-mono ${
                stats.costRate > 30 ? 'text-rose-500' : 'text-emerald-500'
              }`}
            >
              {stats.costRate.toFixed(1)}%
            </div>
          </div>
        </div>
        ...
        {item.tags && item.tags.length > 0 && (
          <div className="flex gap-2 flex-wrap">
            {item.tags.map((tag) => (
              <span
                key={tag}
                className="px-3 py-1 bg-slate-800 border border-slate-700 rounded-full text-xs text-slate-300 flex items-center gap-1"
              >
          ...
          <ul className="space-y-3">
            {item.ingredients.map((ingItem, idx) => {
              ...
              return ing ? (
                <li
                  key={idx}
                  className="flex justify-between items-center text-slate-300 border-b border-slate-800/50 pb-2"
                >
        ...
        <div>
          ...
          <div className="mt-4 flex gap-2">
            ...
            {item.allergens && (
              <span className="text-xs bg-rose-900/20 text-rose-500 px-2 py-1 rounded border border-rose-900/50">
                警示: {item.allergens}
              </span>
            )}
          </div>
        ...
        {item.history?.length > 0 && (
          <div className="pt-6 border-t border-slate-800">
            <details>
              <summary className="text-slate-600 text-xs cursor-pointer hover:text-slate-400 transition">
                查看修改紀錄
              </summary>
        ...
        <div className="pt-6 border-t border-slate-800 flex gap-3">
          <button
            ...
            className="flex-1 py-3 bg-amber-600 hover:bg-amber-500 text-white rounded-xl font-bold text-sm tracking-wide shadow-lg shadow-amber-900/20 flex items-center justify-center gap-2"
          >
            <Edit3 size={18} /> 編輯酒譜
          </button>
          <button
            onClick={() => requestDelete(item.id, 'recipe')}
            className="px-4 py-3 text-rose-500 hover:bg-rose-900/20 border border-rose-900/50 rounded-xl"
          >
            <Trash2 size={20} />
          </button>
        </div>
...
function MainAppContent() {
  ...
  const [availableTechniques, setAvailableTechniques] =
    useState(DEFAULT_TECHNIQUES);
  ...
  const requestDelete = (id, type) => {
    if (type === 'recipe') {
      ...
      if (usedIn.length > 0) {
        ...
        showAlert(
          '無法刪除此材料',
          `此材料 (${targetIng?.nameZh}) 正被以下酒譜使用中：\n\n${names}\n\n請先修改這些酒譜並移除此材料，才能進行刪除。`
        );
        return;
      }
      showConfirm(
        '刪除庫存材料',
        `確定要刪除「${targetIng?.nameZh}」嗎？`,
        () => {
  ...
  const saveItem = () => {
    ...
    if (editorMode === 'ingredient') {
      const newItem = {
        ...
        volume: parseFloat(editingItem.volume) || 0,
      };
      setIngredients((prev) => {
      ...
      const newItem = {
        ...editingItem,
        customPrice: parseFloat(editingItem.customPrice) || 0,
      };
      setRecipes((prev) => {
        ...
        if (exists) {
          ...
          newItem.history = [
            { date: new Date().toISOString(), snapshot: oldVersion },
            ...(exists.history || []),
          ];
      ...
      {activeTab === 'tools' && (
        <div className="p-6 text-center space-y-6 pt-20 w-full">
          ...
          <div className="space-y-3">
            <button
              onClick={() => {
                const data = JSON.stringify({ ingredients, recipes });
            ...
            >
              ...
              <div className="text-left">
                <div className="text-slate-200 font-bold">匯出數據</div>
                <div className="text-xs text-slate-500">備份到手機</div>
              </div>
            </button>
            <label className="w-full bg-slate-800 border border-slate-700 p-4 rounded-xl flex items-center gap-4 hover:bg-slate-700 transition cursor-pointer">
              ...
              <div className="text-left">
                <div className="text-slate-200 font-bold">匯入數據</div>
                <div className="text-xs text-slate-500">從 JSON 還原</div>
              </div>
              <input
                type="file"
                className="hidden"
              ...
              <div className="flex gap-3 w-full pt-2">
                ...
                <button
                  onClick={() => {
                    ...
                    if (dialog.type !== 'confirm') closeDialog();
                  }}
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-950/90 backdrop-blur border-t border-slate-800 pb-safe z-30 w-full">
        <div className="w-full flex justify-around items-center h-16 w-full">
          {[
            ...
            { id: 'quick', icon: Calculator, label: '純飲速算' },
