    ...
    recipe.ingredients.forEach(item => {
      ...
      if (ing) {
        ...
        if (ing.type !== 'other' && (ing.unit === 'ml' || !ing.unit)) {
          ...
          totalVol += amount;
          totalAlcoholVol += amount * (ing.abv / 100);
        }
      }
    });
  }
      ...
      <div className="sticky top-0 bg-slate-950/95 backdrop-blur z-20 border-b border-slate-800 shadow-md px-4 pt-safe pb-0">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar w-full">
          ...
          {ingCategories.map(cat => (
            <div key={cat.id} className="relative group">
              <button onClick={() => setCategoryFilter(cat.id)} className={`whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-bold transition-all pr-4 select-none ${categoryFilter === cat.id ? 'bg-slate-700 text-white border border-amber-500/50 shadow' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}>{cat.label}</button>
              {!['alcohol', 'soft', 'other'].includes(cat.id) && (
                <button onClick={(e) => { e.stopPropagation(); deleteCategory(cat.id); }} className="absolute -top-1 -right-1 bg-rose-600 text-white rounded-full p-0.5 w-4 h-4 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity text-[8px]"><X size={8} strokeWidth={4}/></button>
              )}
            </div>
          ))}
          {isAddingCat ? (
            <div className="flex items-center bg-slate-800 rounded-full px-2 py-1 border border-slate-600 animate-fade-in">
              <input autoFocus className="bg-transparent text-xs text-white w-16 outline-none" placeholder="分類名稱" value={newCatName} onChange={e => setNewCatName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddCategory()} onBlur={() => { if(!newCatName) setIsAddingCat(false); }} />
              <button onClick={handleAddCategory} className="text-amber-500 ml-1"><Check size={14}/></button>
            </div>
          ) : (
            <button onClick={() => setIsAddingCat(true)} className="p-1.5 bg-slate-800 rounded-full text-slate-500 hover:text-white hover:bg-slate-700"><Plus size={14}/></button>
          )}
        </div>
        ...
        {categoryFilter === 'alcohol' && (
          <div className="flex items-center gap-2 overflow-x-auto pb-2 mt-2 no-scrollbar w-full animate-slide-up">
             ...
             <button onClick={()=>setSubCategoryFilter('all')} className={`whitespace-nowrap px-2 py-1 rounded text-[10px] font-medium transition-colors border ${subCategoryFilter === 'all' ? 'bg-slate-700 border-slate-600 text-white' : 'border-transparent text-slate-500'}`}>全部</button>
             {BASE_SPIRITS.map(spirit => (
                 <button key={spirit} onClick={()=>setSubCategoryFilter(spirit)} className={`whitespace-nowrap px-2 py-1 rounded text-[10px] font-medium transition-colors border ${subCategoryFilter === spirit ? 'bg-slate-700 border-slate-600 text-white' : 'border-transparent text-slate-500'}`}>{spirit.split(' ')[0]}</button>
             ))}
          </div>
        )}
      </div>
      ...
      <div className="p-4 space-y-2 w-full">
        {filteredIngredients.map(ing => (
          <div key={ing.id} className="flex items-center justify-between p-3 bg-slate-800 rounded-lg border border-slate-800 hover:border-slate-600 transition-colors group w-full">
            <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={() => startEdit('ingredient', ing)}>
               ...
               <div>
                 ...
                 <div className="text-slate-500 text-xs">
                   {ing.nameEn}
                   {ing.type === 'alcohol' && ing.subType && <span className="ml-2 text-[10px] bg-slate-700 px-1.5 py-0.5 rounded text-slate-400">{ing.subType.split(' ')[0]}</span>}
                 </div>
               </div>
            </div>
            <div className="flex items-center gap-3">
               ...
               <button onClick={(e) => { e.stopPropagation(); requestDelete(ing.id, 'ingredient'); }} className="p-3 -mr-2 text-slate-600 hover:text-rose-500 hover:bg-rose-900/20 rounded-full transition-colors active:scale-95"><Trash2 size={20} /></button>
            </div>
          </div>
        ))}
        {filteredIngredients.length === 0 && (
          <div className="text-center py-10 text-slate-500 flex flex-col items-center"><Layers size={40} className="mb-2 opacity-20"/><span>此分類無材料</span></div>
        )}
      </div>
    </div>
  );
    ...
    <div className="pb-24 animate-fade-in text-slate-200 w-full">
      <div className="bg-slate-900 sticky top-0 z-20 border-b border-slate-800 p-4 pt-safe">
        ...
        <div className="flex bg-slate-800 p-1 rounded-xl">
          <button onClick={() => setMode('single')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all select-none ${mode === 'single' ? 'bg-slate-700 text-white shadow' : 'text-slate-500'}`}>純飲速算</button>
          <button onClick={() => setMode('draft')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all select-none ${mode === 'draft' ? 'bg-amber-600 text-white shadow' : 'text-slate-500'}`}>雞尾酒草稿 (Draft)</button>
        </div>
      </div>
        ...
        {mode === 'single' ? (
          <>
            <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-xl space-y-4">
               ...
               {costPerMl > 0 && <div className="text-center p-2 bg-slate-900/50 rounded-lg"><span className="text-xs text-slate-500">1ml 成本: </span><span className="text-emerald-400 font-mono font-bold">${costPerMl.toFixed(2)}</span></div>}
            </div>
            <div className="flex gap-4 items-center bg-slate-800 p-4 rounded-xl border border-slate-700">
               ...
               <div className="flex items-center gap-2 flex-1">
                  <div className="relative flex-1"><input type="number" value={rateA} onChange={e => setRateA(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-lg p-2 text-center text-amber-400 font-bold focus:border-amber-500 outline-none"/><span className="absolute right-2 top-2 text-slate-500 text-xs">%</span></div>
                  <div className="relative flex-1"><input type="number" value={rateB} onChange={e => setRateB(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-lg p-2 text-center text-blue-400 font-bold focus:border-blue-500 outline-none"/><span className="absolute right-2 top-2 text-slate-500 text-xs">%</span></div>
               </div>
            </div>
            {costPerMl > 0 && (
               <div className="space-y-3">
                 <div className="grid grid-cols-4 gap-2 text-[10px] text-slate-500 uppercase font-bold px-2">
                    <div>規格</div><div className="text-right">成本</div><div className="text-right text-amber-500">{rateA}% 售價</div><div className="text-right text-blue-400">{rateB}% 售價</div>
                 </div>
                 {[{ label: 'Shot (30ml)', vol: 30 }, { label: 'Glass (50ml)', vol: 50 }, { label: 'Double (60ml)', vol: 60 }, { label: `整瓶 (${volNum}ml)`, vol: volNum }].map(row => {
          ...
          <div className="space-y-4">
             <div className="bg-slate-800 rounded-2xl border border-slate-700 p-4">
                <div className="flex justify-between items-center mb-4"><h3 className="font-bold text-slate-200">臨時配方試算</h3><button onClick={() => setDraftIngs([])} className="text-xs text-rose-400 select-none">清空</button></div>
                    ...
                    return (
                      <div key={idx} className="flex justify-between items-center bg-slate-900/50 p-2 rounded">
                         ...
                         <div className="flex items-center gap-2">
                           <input type="number" className="w-16 bg-slate-800 border border-slate-600 rounded text-center text-sm p-1" value={item.amount} onChange={(e) => { const newDraft = [...draftIngs]; newDraft[idx].amount = e.target.value; setDraftIngs(newDraft); }} />
                ...
                {draftIngs.length > 0 && (
                  <div className="bg-slate-900 p-3 rounded-xl border border-slate-800 grid grid-cols-2 gap-4">
                     <div><div className="text-xs text-slate-500">總成本</div><div className="text-xl font-mono text-emerald-400">${Math.round(draftStats.totalCost)}</div></div>
                     <div><div className="text-xs text-slate-500">建議售價 (25%)</div><div className="text-xl font-mono text-amber-400">${draftStats.suggestedPrice}</div></div>
                  </div>
                )}
             </div>
             <div className="bg-slate-800 rounded-2xl border border-slate-700 p-4">
                ...
                <div className="max-h-60 overflow-y-auto space-y-1 custom-scrollbar">
                   {ingredients.filter(ing => ing.nameZh.includes(ingSearch) || ing.nameEn.toLowerCase().includes(ingSearch.toLowerCase())).map(ing => (
                     <button key={ing.id} onClick={() => setDraftIngs([...draftIngs, { id: ing.id, amount: 30 }])} className="w-full text-left p-2 hover:bg-slate-700 rounded flex justify-between group">
                       <span className="text-slate-300 text-sm">{ing.nameZh}</span><Plus size={14} className="text-slate-500 group-hover:text-amber-500" />
                     </button>
                   ))}
                </div>
             </div>
...
const EditorSheet = ({ mode, item, setItem, onSave, onClose, ingredients, availableTechniques, setAvailableTechniques, availableTags, setAvailableTags, availableGlasses, setAvailableGlasses, requestDelete, ingCategories, setIngCategories }) => {
  ...
  const handleAddCategory = () => {
    if(newCatName.trim()){
      const newId = generateId();
      setIngCategories([...ingCategories, { id: newId, label: newCatName.trim() }]);
      setItem({...item, type: newId}); setNewCatName(''); setShowAddCat(false);
    }
  };
        ...
        {isRecipe && (
          <div className="relative h-48 bg-slate-900 rounded-xl border-2 border-dashed border-slate-800 overflow-hidden flex flex-col items-center justify-center text-slate-600 group hover:border-slate-600 transition-colors">
            {item.image ? (
              ...
              <div className="flex flex-col items-center pointer-events-none"><Camera size={32} className="mb-2"/><span className="text-xs">點擊上傳照片</span></div>
            )}
            {!item.image && <input type="file" accept="image/*" onChange={handleImageUpload} className="absolute inset-0 opacity-0 cursor-pointer" />}
          </div>
        )}

        <div className="space-y-4">
           <div><label className="text-xs text-slate-500 font-bold uppercase ml-1">中文名稱</label><input value={item.nameZh} onChange={e => setItem({...item, nameZh: e.target.value})} className="w-full bg-transparent border-b border-slate-700 py-2 text-xl text-slate-100 placeholder-slate-600 focus:border-amber-500 focus:outline-none" placeholder="例如: 內格羅尼" /></div>
           <div><label className="text-xs text-slate-500 font-bold uppercase ml-1">英文名稱</label><input value={item.nameEn} onChange={e => setItem({...item, nameEn: e.target.value})} className="w-full bg-transparent border-b border-slate-700 py-2 text-lg text-slate-400 placeholder-slate-700 focus:border-slate-500 focus:outline-none font-serif italic" placeholder="e.g. Negroni" /></div>
        </div>

        {!isRecipe ? (
          <div className="bg-slate-900 p-4 rounded-xl space-y-4 border border-slate-800">
             <div className="flex gap-4">
               <div className="flex-1"><label className="text-xs text-slate-500 mb-1 block">價格</label><input type="number" value={item.price} onChange={e => setItem({...item, price: e.target.value})} className="w-full bg-slate-800 rounded p-2 text-white border border-slate-700"/></div>
               <div className="flex-1"><label className="text-xs text-slate-500 mb-1 block">容量/數量</label><input type="number" value={item.volume} onChange={e => setItem({...item, volume: e.target.value})} className="w-full bg-slate-800 rounded p-2 text-white border border-slate-700"/></div>
             </div>
             <div className="flex gap-4">
                <div className="flex-1"><label className="text-xs text-slate-500 mb-1 block">單位 (Unit)</label><select value={item.unit || 'ml'} onChange={e => setItem({...item, unit: e.target.value})} className="w-full bg-slate-800 rounded p-2 text-white border border-slate-700">{UNITS.map(u => <option key={u} value={u}>{u}</option>)}</select></div>
                <div className="flex-1"><label className="text-xs text-slate-500 mb-1 block">酒精 %</label><input type="number" value={item.abv} onChange={e => setItem({...item, abv: e.target.value})} className="w-full bg-slate-800 rounded p-2 text-white border border-slate-700"/></div>
             </div>
             <div className="mt-2">
                 <label className="text-xs text-slate-500 mb-1 block">分類</label>
                 <div className="flex gap-2">
                   <select value={item.type} onChange={e => setItem({...item, type: e.target.value})} className="flex-1 bg-slate-800 rounded p-2 text-white border border-slate-700">{ingCategories.map(cat => <option key={cat.id} value={cat.id}>{cat.label}</option>)}</select>
                   {showAddCat ? <div className="flex items-center bg-slate-800 border border-slate-600 rounded"><input className="w-20 bg-transparent p-1 text-xs outline-none" placeholder="新分類" value={newCatName} onChange={e => setNewCatName(e.target.value)}/><button onClick={handleAddCategory} className="p-1 text-amber-500"><Check size={16}/></button></div> : <button onClick={() => setShowAddCat(true)} className="p-2 bg-slate-800 border border-slate-700 rounded text-slate-400 hover:text-white"><Plus size={20}/></button>}
                 </div>
             </div>
             {item.type === 'alcohol' && (
               <div className="mt-4 pt-4 border-t border-slate-800">
                 <label className="flex items-center gap-3 cursor-pointer group">
                   ...
                   <input type="checkbox" className="hidden" checked={item.isSingle !== false} onChange={e => setItem({...item, isSingle: e.target.checked})} />
             ...
             {item.id && <button onClick={() => requestDelete(item.id, 'ingredient')} className="w-full py-3 text-rose-500 text-sm border border-rose-900/50 rounded mt-4">刪除此材料</button>}
          </div>
        ) : (
          <>
            <div className="space-y-4 bg-slate-900 p-4 rounded-xl border border-slate-800">
              <div className="space-y-2">
                ...
                <div className="flex gap-2 mb-4">
                  <button onClick={() => setItem({...item, type: 'classic'})} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-colors border ${item.type === 'classic' ? 'bg-slate-700 border-slate-500 text-white' : 'bg-transparent border-slate-700 text-slate-500 hover:border-slate-500'}`}>經典 Classic</button>
                  <button onClick={() => setItem({...item, type: 'signature'})} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-colors border ${item.type === 'signature' ? 'bg-amber-900/50 border-amber-500 text-amber-500' : 'bg-transparent border-slate-700 text-slate-500 hover:border-slate-500'}`}>特調 Signature</button>
                </div>
              </div>

              <div className="mb-4">
                 <label className="text-xs text-slate-500 font-bold uppercase block mb-2">基酒 (Base Spirit)</label>
                 <select value={item.baseSpirit || ''} onChange={e => setItem({...item, baseSpirit: e.target.value})} className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-white outline-none"><option value="">選擇基酒...</option>{BASE_SPIRITS.map(base => <option key={base} value={base}>{base}</option>)}</select>
              </div>
              <div>
            ...
            <div>
              <div className="flex justify-between items-end mb-2 border-b border-slate-800 pb-2">
                ...
                <div className="text-right"><div className="text-xs text-slate-500">預估成本</div><div className="text-emerald-400 font-mono">${Math.round(calculateRecipeStats(item, ingredients).totalCost)}</div></div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div><label className="text-xs text-slate-500 block mb-1">自訂售價</label><input type="number" placeholder={`建議 $${calculateRecipeStats(item, ingredients).suggestedPrice}`} value={item.customPrice} onChange={e => setItem({...item, customPrice: e.target.value})} className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-white" /></div>
            </div>
            <div><label className="text-xs text-slate-500 block mb-1">製作步驟 / 備註</label><textarea value={item.method} onChange={e => setItem({...item, method: e.target.value})} className="w-full bg-slate-800 border border-slate-700 rounded p-3 text-slate-300 h-32 placeholder-slate-600" placeholder="1. 冰鎮酒杯..." /></div>
          </>
        )}
  ...
  return (
    <div className="fixed inset-0 z-30 bg-slate-950 flex flex-col animate-scale-in overflow-y-auto w-full h-full">
       <div className="relative h-64 sm:h-80 w-full shrink-0">
         {item.image ? <img src={item.image} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-slate-800 flex items-center justify-center"><Wine size={64} className="text-slate-700"/></div>}
         <button onClick={onClose} className="fixed top-4 left-4 z-50 bg-black/30 backdrop-blur p-2 rounded-full text-white hover:bg-black/50 transition mt-safe" style={{ marginTop: 'env(safe-area-inset-top)' }}><ChevronLeft size={24} /></button>
       ...
       <div className="flex-1 p-6 space-y-8">
          <div className="flex justify-between items-center py-4 border-b border-slate-800">
             <div className="text-center"><div className="text-xs text-slate-500 uppercase tracking-widest mb-1">售價 (Price)</div><div className="text-2xl text-amber-400 font-mono font-bold">${stats.price}</div></div>
             <div className="h-8 w-px bg-slate-800"></div>
             <div className="text-center"><div className="text-xs text-slate-500 uppercase tracking-widest mb-1">酒精 (ABV)</div><div className="text-xl text-slate-300 font-mono">{stats.finalAbv.toFixed(1)}%</div></div>
             <div className="h-8 w-px bg-slate-800"></div>
             <div className="text-center"><div className="text-xs text-slate-500 uppercase tracking-widest mb-1">成本率 (Cost)</div><div className={`text-xl font-mono ${stats.costRate > 30 ? 'text-rose-500' : 'text-emerald-500'}`}>{stats.costRate.toFixed(1)}%</div></div>
          </div>

          {item.flavorDescription && <div className="relative bg-slate-900/50 p-6 rounded-xl border border-slate-800"><Quote className="absolute top-4 left-4 text-slate-700 w-6 h-6" /><div className="italic text-slate-300 text-sm leading-relaxed pl-6">{item.flavorDescription}</div></div>}
          {item.tags && item.tags.length > 0 && <div className="flex gap-2 flex-wrap">{item.tags.map(tag => <span key={tag} className="px-3 py-1 bg-slate-800 border border-slate-700 rounded-full text-xs text-slate-300 flex items-center gap-1"><Tag size={12} className="text-amber-500"/> {tag}</span>)}</div>}

          <div>
            <h3 className="text-amber-500 text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2"><GlassWater size={14}/> 配方 (Recipe)</h3>
            <ul className="space-y-3">
              {item.ingredients.map((ingItem, idx) => {
                const ing = ingredients.find(i => i.id === ingItem.id);
                return ing ? <li key={idx} className="flex justify-between items-center text-slate-300 border-b border-slate-800/50 pb-2"><span>{ing.nameZh}</span><span className="font-mono text-slate-500">{ingItem.amount}{ing.unit || 'ml'}</span></li> : null;
              })}
            </ul>
            <div className="mt-4 p-3 bg-slate-900 rounded border border-slate-800 flex justify-between text-xs text-slate-500 font-mono"><span>總成本: ${stats.totalCost.toFixed(1)}</span><span>毛利: ${stats.margin.toFixed(0)}</span></div>
          </div>

          <div>
             <h3 className="text-amber-500 text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2"><Settings size={14}/> 製作說明 (Method)</h3>
             <div className="text-slate-300 leading-relaxed whitespace-pre-wrap font-serif">{item.method || "無製作說明"}</div>
             <div className="mt-4 flex gap-2">
                <span className="text-xs bg-slate-800 text-slate-400 px-2 py-1 rounded">杯型: {item.glass}</span>
                {item.allergens && <span className="text-xs bg-rose-900/20 text-rose-500 px-2 py-1 rounded border border-rose-900/50">警示: {item.allergens}</span>}
             </div>
          </div>

          {item.history?.length > 0 && (
             <div className="pt-6 border-t border-slate-800">
                <details><summary className="text-slate-600 text-xs cursor-pointer hover:text-slate-400 transition">查看修改紀錄</summary><div className="mt-3 space-y-2">{item.history.map((h, i) => <div key={i} className="text-[10px] text-slate-500 flex justify-between"><span>{formatDate(h.date)}</span><span>舊版本</span></div>)}</div></details>
             </div>
          )}
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-amber-500/30 w-full overflow-x-hidden">
      ...
      {activeTab === 'tools' && (
         <div className="p-6 text-center space-y-6 pt-20 w-full">
           ...
           <h2 className="text-xl font-serif text-slate-200">Bar Manager v7.0</h2>
           <div className="space-y-3">
